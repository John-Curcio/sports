...
class NonSymmetricModel(torch.nn.Module):
    
    def __init__(self, input_dim, width_1, width_2, width_3):
        super().__init__()
        self.linear1 = torch.nn.Linear(input_dim, width_1)
        self.activation = torch.nn.Tanh() # tanh(x) = -tanh(-x)
        self.linear2 = torch.nn.Linear(width_1, width_2)
        # take diff
        self.linear3 = torch.nn.Linear(width_2*2, width_3)
        self.linear4 = torch.nn.Linear(width_3, 1)
        self.softmax = torch.nn.Sigmoid()
    ...
    def forward(self, ml_logit, x_f, x_o):
        ...
        x_o = self.activation(x_o)
        # concat
        x = torch.concat((x_f, x_o), 1)
        x = self.linear3(x)
        x = self.activation(x)
        x = self.linear4(x)
        y_hat = self.softmax(ml_logit + x)
        return y_hat
    

class NonSymmetricModelWrapper(object):
    
    def __init__(self, fighter_cols, opponent_cols, n_pca=16, 
                 width_1=16, width_2=16, width_3=16, lr=0.001, n_epochs=30, batch_size=64):
        self.fighter_cols = fighter_cols
        self.opponent_cols = opponent_cols
        assert len(fighter_cols) == len(opponent_cols) # fuck style
        self.n_pca = n_pca
    ...
    def _get_doubled_data(self, X_f, X_o, y, ml_logit):
        X_f_double = np.concatenate([X_f, X_o])
        X_o_double = np.concatenate([X_o, X_f])
        y_double = np.concatenate([y, 1-y])
        ml_logit_double = np.concatenate([ml_logit, -1 * ml_logit])
        return X_f_double, X_o_double, y_double, ml_logit_double
    
    def _train(self, X_f_train, X_o_train, y_train, ml_logit_train):
        X_f_train, X_o_train, y_train, ml_logit_train = self._get_doubled_data(
            X_f_train, X_o_train, y_train, ml_logit_train
        )
        input_dim = X_f_train.shape[1]
        self._model = NonSymmetricModel(input_dim, self.width_1, self.width_2, self.width_3)
        optimizer = torch.optim.Adam(self._model.parameters(), lr=self.lr)
        loss_fn = torch.nn.BCELoss() # binary cross entropy
        for epoch in tqdm(range(self.n_epochs)):
            # shuffle dataset
    ...
    def fit_predict(self, train_df, test_df):
        ...
        y_hat2 = self._model(
            torch.Tensor(-1*ml_logit_test).view(-1,1), 
            torch.Tensor(X_o_test),
            torch.Tensor(X_f_test),
        )
        y_hat = (
            y_hat1.detach().numpy()[:,0] +
            (1 - y_hat2.detach().numpy()[:,0])
        ) / 2
        return y_hat